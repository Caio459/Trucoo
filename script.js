document.addEventListener('DOMContentLoaded', () => {
    ...
    function endRound(winner) { roundInProgress = false; if (winner === 'player') { playerScore += roundValue; } else if (winner === 'computer') { computerScore += roundValue; } roundStarter = (roundStarter === 'player') ? 'computer' : 'player'; updateScores(); if (playerScore >= 12) { endMatch('player'); } else if (computerScore >= 12) { endMatch('computer'); } else { gameMessageEl.textContent = `Fim da rodada.`; setTimeout(renderActionButtons, 2000); } }
    ...
    function handleTrucoRequest(who) {
        ...
        renderActionButtons();
        if (who === 'player') setTimeout(computerRespondToTruco, 1500);
    }

    function computerRespondToTruco() {
        const handStrength = Math.max(...computerHand.map(getCardStrength));
        const nextBet = roundValue + 3;

        // Cenário 1: Mão excelente, aumenta a aposta
        ...
        // Cenário 2: Mão boa, aceita
        else if (handStrength >= 11) {
            trucoState = 'none';
            gameMessageEl.textContent = `O computador ACEITOU! A rodada vale ${roundValue} pontos.`;
            renderActionButtons();
            // O jogo continua, a vez volta para quem trucou
            if (challenger === 'player') {
                isPlayerTurn = false;
                setTimeout(computerPlayCard, 1000);
            }
        } 
        // Cenário 3: Mão fraca, corre
        else {
            const pointsWon = 1; // Se corre do TRUCO, é sempre 1 ponto
            ...
            else { setTimeout(renderActionButtons, 2000); }
        }
    }

    function playerRespondToTruco(response) {
        if (response === 'run') {
            const pointsWon = roundValue === 3 ? 1 : roundValue - 3; // Lógica correta de pontos
            computerScore += pointsWon;
            gameMessageEl.textContent = `Você CORREU! O computador ganhou ${pointsWon} ponto(s).`;
            roundInProgress = false;
            trucoState = 'none';
            updateScores();
            if (computerScore >= 12) { endMatch('computer'); }
            else { setTimeout(renderActionButtons, 2000); }
        } else if (response === 'accept') {
            trucoState = 'none';
            gameMessageEl.textContent = `Você ACEITOU! A rodada vale ${roundValue} pontos.`;
            renderActionButtons();
            // A vez de jogar volta para o desafiante (o computador)
            if (challenger === 'computer') {
                isPlayerTurn = true;
                gameMessageEl.textContent += ' Sua vez!';
            }
