document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateRoundValueDisplay() {
        if (roundValue > 1) {
            roundValueDisplayEl.textContent = `VALENDO ${roundValue} PONTOS`;
            roundValueDisplayEl.classList.remove('hidden');
        } else {
            roundValueDisplayEl.classList.add('hidden');
        }
    }
    ...
    function computerRespondToTruco() {
        ...
        if (handStrength >= 13 && nextBet <= 12) { roundValue = nextBet; challenger = 'computer'; trucoState = 'pending_player_response'; gameMessageEl.textContent = `O computador pediu ${nextBet === 6 ? 'SEIS' : 'NOVE'}!`; updateRoundValueDisplay(); renderActionButtons(); }
        else if (handStrength >= 11) { trucoState = 'none'; gameMessageEl.textContent = `O computador ACEITOU!`; renderActionButtons(); if (challenger === 'player') { isPlayerTurn = false; setTimeout(computerPlayCard, 1000); } else { isPlayerTurn = true; } }
        else { const pointsWon = roundValue === 3 ? 1 : roundValue - 3; playerScore += pointsWon; gameMessageEl.textContent = `O computador CORREU! Você ganhou ${pointsWon} ponto.`; roundInProgress = false; trucoState = 'none'; updateScores(); if (playerScore >= 12) { endMatch('player'); } else { setTimeout(renderActionButtons, 2000); } }
    }
    ...
    function playerRespondToTruco(response) {
        ...
        else if (response === 'accept') { trucoState = 'none'; gameMessageEl.textContent = `Você ACEITOU! A rodada vale ${roundValue} pontos.`; renderActionButtons(); if (challenger === 'computer') { isPlayerTurn = true; } else { isPlayerTurn = false; setTimeout(computerPlayCard, 1000); } }
        else if (response === 'raise') { const nextBet = roundValue + 3; if (nextBet <= 12) { roundValue = nextBet; challenger = 'player'; trucoState = 'pending_computer_response'; gameMessageEl.textContent = `Você pediu ${nextBet === 6 ? 'SEIS' : 'NOVE'}!`; updateRoundValueDisplay(); renderActionButtons(); setTimeout(computerRespondToTruco, 1500); } }
    }
    ...
    function renderActionButtons() {
        ...
        if (trucoState === 'pending_player_response') {
            ...
            // CORREÇÃO: O botão de truco só aparece se não houver uma aposta em andamento
            if (trucoState === 'none') {
