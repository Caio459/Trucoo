document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTrickHistory() {
        const results = trickResultsEl.children;
        for (let i = 0; i < 3; i++) {
            const winner = state.trickWinners[i];
            if (winner) {
                // MUDANÇA AQUI
                results[i].textContent = `${i+1}º: ${winner === 'player' ? 'Você' : winner === 'computer' ? 'MARCOS' : 'Empate'}`;
            } else {
                results[i].textContent = `${i+1}º: -`;
            }
        }
    }
    ...
    function playerPlayCard(card) { if (!state.isPlayerTurn || !state.roundInProgress || state.trucoState !== 'none') return; state.playerCardOnTable = card; state.playerHand = state.playerHand.filter(c => c !== card); renderPlayerHand(); renderPlayedCard(card, playerCardPlayedEl); state.isPlayerTurn = false; if (!state.computerCardOnTable) { gameMessageEl.textContent = 'Vez do MARCOS...'; setTimeout(computerPlayCard, 1000); } else { setTimeout(evaluateTrick, 1000); } }
    ...
    function evaluateTrick() {
        ...
        if (playerStrength > computerStrength) { winnerOfThisTrick = 'player'; } else if (computerStrength > playerStrength) { winnerOfThisTrick = 'computer'; } else { winnerOfThisTrick = 'tie'; }
        state.trickWinners[state.currentTrick - 1] = winnerOfThisTrick;
        if(state.currentTrick === 1 && winnerOfThisTrick !== 'tie') { state.firstTrickWinner = winnerOfThisTrick; }
        renderTrickHistory();
        gameMessageEl.textContent = winnerOfThisTrick === 'player' ? 'Você ganhou o turno!' : winnerOfThisTrick === 'computer' ? 'MARCOS ganhou o turno!' : 'Empatou o turno!';
    ...
    function checkForRoundEnd() {
        ...
        if (state.trickWinners[0] === 'tie') { if (state.trickWinners.length > 1 && state.trickWinners[1] !== 'tie') roundWinner = state.trickWinners[1]; }
        if (state.currentTrick === 2 && state.trickWinners[1] === 'tie' && state.firstTrickWinner) { roundWinner = state.firstTrickWinner; }
        if (state.currentTrick === 3) { if (playerWins > computerWins) roundWinner = 'player'; else if (computerWins > playerWins) roundWinner = 'computer'; else roundWinner = state.firstTrickWinner || state.roundStarter; }
        if (roundWinner) { endRound(roundWinner); }
        else { startNextTrick(state.trickWinners[state.currentTrick - 1]); }
    }
    ...
    function startNextTrick(winner) {
        ...
        else { gameMessageEl.textContent = `Turno ${state.currentTrick}: Vez do MARCOS...`; setTimeout(computerPlayCard, 1500); }
    }
    ...
    function endRound(winner) {
        ...
        if (state.playerScore >= 12) { endMatch('player'); }
            ...
            let winnerText = winner === 'tie' ? 'A rodada empatou!' : `${winner === 'player' ? 'Você' : 'MARCOS'} ganhou a rodada!`;
    ...
    function endMatch(winner) {
        ...
        else if (state.computerMatchesWon === 2) { endSeries('MARCOS'); }
        else { state.gameInProgress = false; gameMessageEl.textContent = `${winner === 'player' ? 'Você' : 'MARCOS'} ganhou a partida!`; renderActionButtons(); }
    }
    ...
    function updateScores() {
        ...
        playerMatchesWonEl.textContent = state.playerMatchesWon;
        computerMatchesWonEl.textContent = state.computerMatchesWon;
    }

    function resetSeries() { setupInitialState(); updateScores(); startNewMatch(true); }
    function startNewMatch(isNewSeries = false) { state.playerScore = 0; state.computerScore = 0; state.roundStarter = isNewSeries ? 'player' : state.roundStarter; state.gameInProgress = true; startNewRound(); }
    ...
    function startNewRound() {
        ...
        renderActionButtons();
        if (!state.isPlayerTurn) { gameMessageEl.textContent = 'Turno 1: Vez do MARCOS...'; setTimeout(computerPlayCard, 1500); }
        else { gameMessageEl.textContent = 'Turno 1: Sua vez de jogar!'; }
    }
    ...
    function handleTrucoRequest(who) {
        ...
        state.challenger = who; state.roundValue = 3;
        ...
        if (who === 'player') {
            ...
            gameMessageEl.textContent = 'MARCOS pediu TRUCO!';
        }
        renderActionButtons();
    }
    ...
    function computerRespondToTruco() {
        ...
        if (handStrength >= 13 && nextBet <= 12) {
            state.roundValue = nextBet; state.challenger = 'computer'; state.trucoState = 'pending_player_response';
            gameMessageEl.textContent = `MARCOS pediu ${nextBet === 6 ? 'SEIS' : 'NOVE'}!`;
        } else if (handStrength >= 11) {
            state.trucoState = 'none';
            gameMessageEl.textContent = `MARCOS ACEITOU! A rodada vale ${state.roundValue} pontos.`;
            updateRoundValueDisplay();
            if (state.challenger === 'player') {
                state.isPlayerTurn = true;
                gameMessageEl.textContent += ' Sua vez de jogar!';
            }
        } else {
            const pointsWon = 1; state.playerScore += pointsWon;
            gameMessageEl.textContent = `MARCOS CORREU! Você ganhou ${pointsWon} ponto.`;
    ...
    function playerRespondToTruco(response) {
        if (response === 'run') { const pointsWon = state.roundValue === 3 ? 1 : state.roundValue - 3; state.computerScore += pointsWon; gameMessageEl.textContent = `Você CORREU! MARCOS ganhou ${pointsWon} ponto(s).`; state.roundInProgress = false; state.trucoState = 'none'; updateScores(); if (state.computerScore >= 12) { endMatch('computer'); } else { renderActionButtons(); } }
        else if (response === 'accept') { state.trucoState = 'none'; gameMessageEl.textContent = `Você ACEITOU! A rodada vale ${state.roundValue} pontos.`; updateRoundValueDisplay(); if (state.challenger === 'computer') { state.isPlayerTurn = false; gameMessageEl.textContent += ' Vez do MARCOS!'; setTimeout(computerPlayCard, 1000); } }
        else if (response === 'raise') { const nextBet = state.roundValue + 3; if (nextBet <= 12) { state.roundValue = nextBet; state.challenger = 'player'; state.trucoState = 'pending_computer_response'; gameMessageEl.textContent = `Você pediu ${nextBet === 6 ? 'SEIS' : 'NOVE'}!`; renderActionButtons(); setTimeout(computerRespondToTruco, 1500); } }
        renderActionButtons();
    }
