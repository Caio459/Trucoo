document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTrickHistory() {
        ...
        for (let i = 0; i < 3; i++) {
            const winner = trickWinners[i];
            if (winner) {
                results[i].textContent = `${i+1}º: ${winner === 'player' ? 'Você' : winner === 'computer' ? 'CPU' : 'Empate'}`;
            } else {
    ...
    function computerRespondToTruco() {
        ...
        if (handStrength >= 13 && nextBet <= 12) {
            ...
            trucoState = 'none';
            gameMessageEl.textContent = `O computador ACEITOU!`;
            updateRoundValueDisplay(); // MOSTRA O PLACAR AGORA
            renderActionButtons();
    ...
    function playerRespondToTruco(response) {
        if (response === 'run') {
            ...
            trucoState = 'none';
            gameMessageEl.textContent = `Você ACEITOU!`;
            updateRoundValueDisplay(); // MOSTRA O PLACAR AGORA
            renderActionButtons();
